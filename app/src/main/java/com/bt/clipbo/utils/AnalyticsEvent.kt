...
class AnalyticsManager @Inject constructor(
    ...
    init {
        analyticsScope.launch {
            ...
            // Start session
            trackEvent(AnalyticsEvent.AppLaunched, AnalyticsData(
                parameters = mapOf(
    ...
    fun trackEvent(event: AnalyticsEvent, data: AnalyticsData = AnalyticsData()) {
        if (!isAnalyticsEnabled) return

        analyticsScope.launch {
            try {
                val eventEntity = UsageAnalyticsEntity(
    ...
    fun trackClipboardOperation(
        ...
        trackEvent(event, AnalyticsData(
            parameters = mapOf(
    ...
    fun trackSearch(query: String, resultCount: Int, filters: List<String>) {
        trackEvent(AnalyticsEvent.SearchPerformed, AnalyticsData(
            parameters = mapOf(
    ...
    fun trackError(
        ...
        trackEvent(AnalyticsEvent.ErrorOccurred, AnalyticsData(
            parameters = mapOf(
    ...
    fun trackPerformance(
        ...
        if (duration > 1000) { // Only track slow operations (>1 second)
            trackEvent(AnalyticsEvent.DatabaseQuerySlow, AnalyticsData(
                parameters = mapOf(
    ...
    fun trackMemoryWarning(
        ...
        trackEvent(AnalyticsEvent.MemoryWarning, AnalyticsData(
            parameters = mapOf(
    ...
    fun trackSettingChange(settingName: String, oldValue: Any?, newValue: Any?) {
        trackEvent(AnalyticsEvent.SettingChanged, AnalyticsData(
            parameters = mapOf(
